...
class MinIoURandomCrop(object):
    ...
    def __init__(self,crop_ratio = 0.5, min_ious=(0.1, 0.3, 0.5, 0.7, 0.9), min_crop_size=0.3):
    ...
    def __call__(self, results):
        if 'crop' not in results:
            crop = True if np.random.rand() < self.crop_ratio else False
            results['crop'] = crop

        if results['crop']:
            #print('start MinIoURandomCrop')
            img, boxes, labels = [
...
class RandomTranslate(object):
    """Translate the image & bbox.

    If the input dict contains the key "translate", then the flag will be used,
    otherwise it will be randomly decided by a ratio specified in the init
    method.

    Args:
        translate_ratio (float, optional): The flipping probability.
    """

    def __init__(self, translate_ratio=None, translate = 0.2, diff = False):
        self.translate_ratio = translate_ratio
        self.translate = translate
        self.diff = diff
        if translate_ratio is not None:
            assert translate_ratio >= 0 and translate_ratio <= 1
    ...
    def translate_img(self,img,bboxes):
        bws = bboxes[:,2] - bboxes[:,0]
        bhs = bboxes[:,3] - bboxes[:,1]
        #print((bws == 0).any())

        if (bws == 0).any() or (bhs == 0).any():
            print('Not translate')
            return img,bboxes
        ...
        translate_factor_x = np.random.uniform(-self.translate,self.translate)
        translate_factor_y = np.random.uniform(-self.translate,self.translate)
        ...
        bboxes = self.clip_box(bboxes, [0, 0, img_shape[1], img_shape[0]], 0.25)
        return img, bboxes

    def __call__(self, results):
        if 'translate' not in results:
            translate = True if np.random.rand() < self.translate_ratio else False
        ...
        if results['translate']:
            ...
            for key in results.get('bbox_fields', []):
                bboxes = results[key]
                if len(bboxes) > 0:
                    results['img'],results[key] = self.translate_img(results['img'],results[key])

        return results

    def __repr__(self):
        return self.__class__.__name__ + '(translate_ratio={},translate = {},diff = {})'.format(
            self.translate_ratio,self.translate,self.diff)
...
class BBoxJitter(object):
    ...
    def __init__(self,jitter_ratio =0.5, min=0, max=2):
        ...
        if jitter_ratio is not None:
            assert jitter_ratio >= 0 and jitter_ratio <= 1
    ...
    def __call__(self, results):
        if 'jitter' not in results:
            jitter = True if np.random.rand() < self.jitter_ratio else False
    ...
    def __repr__(self):
        return self.__class__.__name__ + '(jitter_ratio={},min_scale={},max_scale={})'.format(
            self.jitter_ratio,self.min_scale, self.max_scale)


@PIPELINES.register_module
class RandomShear(object):
    """Shear the image & bbox.

    If the input dict contains the key "shear", then the flag will be used,
    otherwise it will be randomly decided by a ratio specified in the init
    method.

    Args:
        shear_ratio (float, optional): The flipping probability.
    """

    def __init__(self, shear_ratio=None, shear_factor = 0.2):
        self.shear_ratio = shear_ratio
        self.shear_factor = shear_factor
        if shear_ratio is not None:
            assert shear_ratio >= 0 and shear_ratio <= 1
    def shear_img(self,img,bboxes):
        
        shear_factor = np.random.uniform(0,self.shear_factor)
    ...
    def __call__(self, results):
        if 'shear' not in results:
            shear = True if np.random.rand() < self.shear_ratio else False
            results['shear'] = shear

        if results['shear']:
            # shear image and bboxes
            for key in results.get('bbox_fields', []):
                results['img'],results[key] = self.shear_img(results['img'],results[key])

        return results

    def __repr__(self):
        return self.__class__.__name__ + '(shear_ratio={},shear_factor = {})'.format(
            self.shear_ratio,self.shear_factor)


@PIPELINES.register_module
class RandomMixup(object):
    """Mixup two images & bbox.

    If the input dict contains the key "mixup", then the flag will be used,
    otherwise it will be randomly decided by a ratio specified in the init
    method.

    Args:
        mixup_ratio (float, optional): The mixuping probability.
    """

    def __init__(self, mixup_ratio=None, lambd = 0.5):
        ...
        self.boxes2 = []
        self.labels2 = []
        if mixup_ratio is not None:
            assert mixup_ratio >= 0 and mixup_ratio <= 1
    def mixup_imgs(self,results):
        self.count += 1
        img, boxes, labels = [
                results[k] for k in ('img', 'gt_bboxes', 'gt_labels')
            ]
        ...
        if self.count % 2 == 0 and self.count != 0:
            ...
            width = max(self.img1.shape[1],self.img2.shape[1])

            mix_img = np.zeros(shape=(height,width,3),dtype='float32')
            mix_img[:self.img1.shape[0],:self.img1.shape[1],:] = self.img1.astype('float32')*self.lambd
            mix_img[:self.img2.shape[0],:self.img2.shape[1],:] += self.img2.astype('float32')*(1.0 - self.lambd)
            mix_img = mix_img.astype('uint8')

            mix_boxes = np.vstack((self.boxes1,self.boxes2))
            mix_labels = np.concatenate((self.labels1,self.labels2))

            results['img'] = mix_img
            results['gt_bboxes'] = mix_boxes
            results['gt_labels'] = mix_labels
        
        return results

    def __call__(self, results):
        if 'mixup' not in results:
            mixup = True if np.random.rand() < self.mixup_ratio else False
            results['mixup'] = mixup
    ...
    def __repr__(self):
        return self.__class__.__name__ + '(mixup_ratio={},lambd = {})'.format(
            self.mixup_ratio,self.lambd)

@PIPELINES.register_module
class AlbuSizedBBoxSafeCrop(object):
    ...
    def __call__(self, results):
        ...
        img_h,img_w,img_c = results['img_shape']
        for index,value in enumerate(self.transforms):
            self.transforms[index]['height'] = img_h
            self.transforms[index]['width'] = img_w
